...
function sample!(lahmc::LAHMC)
    ...
    for i in 1:lahmc.n_samples-1
        ...
        for j in 1:lahmc.K
            proposed_q, p = leapfrog(Z_chain[end], p, lahmc.epsilon, lahmc.L, call_dU!(lahmc))
            push!(Z_chain, proposed_q)
            
            p_cum, Cl = leap_prob_recurse(Z_chain, C[1:j+1, 1:j+1, active_idx], active_idx, lahmc.U)
            ...
            lahmc.samples[:, accepted_idx, i+1] = Z_chain[end][:, accepted_idx]
...
function leap_prob_recurse(Z_chain, C, active_idx, U)
	"""
	...
	if length(Z_chain) == 2
		...
		H0 = U(Z_chain[1]) .- 0.5 * sum(Z_chain[1].^2, dims=1)
    	H1 = U(Z_chain[2]) .- 0.5 * sum(Z_chain[2].^2, dims=1)
        diff = H0 .- H1
	...
	cum_forward, Cl = leap_prob_recurse(Z_chain[1:end-1], C[1:end-1, 1:end-1, :], active_idx, U)
	C[1:end-1,1:end-1, :] = Cl
	cum_reverse, Cl = leap_prob_recurse(Z_chain[end:-1:2], C[end:-1:2, end:-1:2, :], active_idx, U)
	C[end:-1:2, end:-1:2, :] = Cl

	H0 = U(Z_chain[1]) .- 0.5 * sum(Z_chain[1].^2, dims=1)
    H1 = U(Z_chain[end]) .- 0.5 * sum(Z_chain[end].^2, dims=1)
    diff = H0 .- H1
    diff = diff[:, active_idx]
	start_state_ratio = exp.(diff)
